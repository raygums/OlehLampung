...
<div class="container-main py-8">
    <h1 class="font-heading text-2xl font-bold text-navy mb-2">Riwayat Pesanan</h1>
    <p class="text-gray-500 text-sm mb-6">Lacak dan pantau semua pesanan Anda.</p>
    ...
    @if($orders->count())
        <div class="space-y-4">
            @foreach($orders as $order)
                <a href="{{ route('orders.show', $order) }}" class="block bg-white rounded-xl border border-gray-200 p-5 hover:border-amber hover:shadow-md transition-all">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">{{ $order->created_at->translatedFormat('d M Y, H:i') }} · {{ $order->items->count() }} item</p>
                        </div>
                        <div class="text-right">
                            <p class="font-bold text-amber text-lg">{{ $order->formatted_total }}</p>
                            <p class="text-xs text-gray-400">{{ $order->full_name }}</p>
                        </div>
                    </div>
                </a>
            @endforeach
