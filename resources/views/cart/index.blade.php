...
<div class="container-main py-8">
    <h1 class="font-heading text-2xl font-bold text-navy mb-6">Keranjang Belanja</h1>
        ...
        <div class="grid md:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <table class="w-full">
                        <thead class="bg-gray-50 border-b">
                            <tr>
                                <th class="text-left text-xs font-semibold text-gray-500 uppercase px-6 py-3">Produk</th>
                                <th class="text-center text-xs font-semibold text-gray-500 uppercase px-4 py-3">Jumlah</th>
                                <th class="text-right text-xs font-semibold text-gray-500 uppercase px-6 py-3">Subtotal</th>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-400 mt-1">{{ $item['product']->formatted_price }} / pcs</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-center border border-gray-300 rounded-lg mx-auto w-fit">
                                            <button onclick="updateQty({{ $item['product']->id }}, -1)" class="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-navy">−</button>
                                            <input type="number" value="{{ $item['quantity'] }}" min="1" class="w-10 text-center border-x border-gray-300 h-8 text-sm font-semibold qty-input" data-id="{{ $item['product']->id }}">
                                            <button onclick="updateQty({{ $item['product']->id }}, 1)" class="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-navy">+</button>
                                        </div>
                                    </td>
            ...
            <div>
                <div class="bg-white rounded-xl border border-gray-200 p-6 sticky top-20">
                    <h3 class="font-heading font-semibold text-navy mb-4">Ringkasan Belanja</h3>
